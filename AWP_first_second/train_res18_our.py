...
parser.add_argument('--model-dir', default='./model-cifar10-resnet18/ours',
...
def train(model, train_loader, optimizer, epoch, awp_adversary):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        def reg_gap(tl_gap):
            ...
            for i in range(len(tt)):
                tt[i,i]=0.
            tt = 0.8*tt/tt.sum(1).reshape(-1,1)
            for i in range(len(tt)):
                tt[i,i]=0.2
            return tt
        ...
        loss = loss_natural*(1-args.alpha) + args.alpha*0.5*(loss_tl1+loss_tl1_) + (0.5*args.alpha*0.4)*(loss_tl2+loss_tl2_) + args.beta*loss_robust
...
def eval_adv_test(model, device, test_loader):
    ...
    with torch.no_grad():
        for data, target in test_loader:
            ...
            atk = torchattacks.PGDL2(copy.deepcopy(model), eps=epsilon, alpha=step_size, steps=args.num_steps)
            data_pgd = atk(X, y)
...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs + 1):
        ...
        # save checkpoint
        if (epoch>99 and epoch%5==0) or (epoch>99 and epoch<110) or (epoch>149 and epoch<160):
